...
VOICE_PARAMETER_RANGES = {
    f"{i}_{key}": value
    for key, value in OSCILLATOR_VALID_RANGES.items()
...
VOICE_KEYS = [f"{i}_{key}" for i in range(6) for key in OSC_KEYS] + GENERAL_KEYS

KEYS = HEADER_KEYS + list(VOICE_KEYS * N_VOICES) + FOOTER_KEYS
...
class DX7Single:
    ...
    def to_syx(voices):
        ...
        data = bytes(DX7Single.HEADER) + voices_bytes + bytes(patch_checksum)
